Option Explicit
    ...
    Public Sub setDisplay(i As Long)
        Me.slipNum = Cells(i, 1).Value
        Me.slipDay = Cells(i, 2).Value
'        科目設定
        If Cells(i, 10).Value <> "" Then
            Me.debitName = Cells(i, 7).Value
            Me.debitAmo = Cells(i, 10).Value
            
        Else
            Me.creditName = Cells(i, 7).Value
            Me.creditAmo = Cells(i, 11).Value - Cells(i, 11).Value - Cells(i, 11).Value
        End If
        
        Me.summary = Cells(i, 9).Value
